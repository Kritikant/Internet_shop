<?php
/*
Plugin Name: WooCommerce Alfabank Checkout plugin
Plugin URI:
Description: Allows to use a payment gateway with the WooCommerce.
Version: 4.1.4
Author: RBSPayment
Text Domain: wc-alfabank-text-domain
Domain Path: /lang
*/
if (!defined('ABSPATH')) exit;
require_once(ABSPATH . 'wp-admin/includes/plugin.php');
require_once(__DIR__ . '/include.php');
if (file_exists(__DIR__ . '/AlfabankDiscount.php')) {
include(__DIR__ . '/AlfabankDiscount.php');
}
add_filter('plugin_row_meta', 'alfabank_register_plugin_links', 10, 2);
function alfabank_register_plugin_links($links, $file)
{
$base = plugin_basename(__FILE__);
if ($file == $base) {
$links[] = '<a href="admin.php?page=wc-settings&tab=checkout&section=alfabank">' . __('Settings', 'woocommerce') . '</a>';
}
return $links;
}
add_action('plugins_loaded', 'woocommerce_alfabank', 0);
function woocommerce_alfabank()
{
load_plugin_textdomain('wc-alfabank-text-domain', false, dirname(plugin_basename(__FILE__)) . '/lang');
if (!class_exists('WC_Payment_Gateway'))
return;
if (class_exists('WC_RBSPayment_Alfabank'))
return;
class WC_RBSPayment_Alfabank extends WC_Payment_Gateway
{
...
$this->fiscale_options = RBSPAYMENT_ALFABANK_ENABLE_CART_OPTIONS;
$this->orderNumberById = true; //false - must be installed WooCommerce Sequential Order Numbers
$this->allowCallbacks = RBSPAYMENT_ALFABANK_ENABLE_CALLBACK;
...
add_action('valid-alfabank-standard-ipn-request', array($this, 'successful_request'));
...
$callback_addresses_string = get_option('siteurl') . "?wc-api=WC_RBSPayment_Alfabank&alfabank=callback";
...
{
if (isset($_GET['alfabank'])) {
$action = $_GET['alfabank'];
if ($this->test_mode == 'yes') {
$action_adr = $this->test_url;
} else {
$action_adr = $this->prod_url;
...
{
?>
<h3><?php echo RBSPAYMENT_ALFABANK_PAYMENT_NAME; ?></h3>
<p><?php _e("Allow customers to conveniently checkout directly with ", 'wc-' . $this->id . '-text-domain'); ?><?php echo RBSPAYMENT_ALFABANK_PAYMENT_NAME; ?></p>
...
'type' => 'checkbox',
'label' => __('Enable', 'woocommerce') . " " . RBSPAYMENT_ALFABANK_PAYMENT_NAME,
'default' => 'yes'
),
'title' => array(
...
if (RBSPAYMENT_ALFABANK_ENABLE_CART_OPTIONS === true) {
...
if (RBSPAYMENT_ALFABANK_CUSTOMER_EMAIL_SEND && !empty($order_data['billing']['email'])) {
...
'password' => $this->password,
'amount' => $amount,
'returnUrl' => get_option('siteurl') . '?wc-api=WC_RBSPayment_Alfabank&alfabank=result&order_id=' . $order_id . $extra_url_param,
'currency' => unserialize(RBSPAYMENT_ALFABANK_CURRENCY_CODES)[get_woocommerce_currency()],
...
'measure' => RBSPAYMENT_ALFABANK_MEASUREMENT_NAME
);
} else {
$item['quantity'] = array(
'value' => $value['quantity'],
'measure' => RBSPAYMENT_ALFABANK_MEASUREMENT_CODE
);
}
...
if (class_exists('AlfabankDiscount')) {
...
$result .= "\n\n";
$path = dirname(__FILE__) . '/wc_alfabank_' . date('Y-m') . '.log';
error_log($information . $result, 3, $path);
return true;
}
return false;
}
function alfabank_change_status_function($order_id)
{
$order = wc_get_order($order_id);
...
if (!function_exists('add_alfabank_gateway')) {
function add_alfabank_gateway($methods)
{
$methods[] = 'WC_RBSPayment_Alfabank';
return $methods;
}
}
...
add_filter('woocommerce_payment_gateways', 'add_alfabank_gateway');
}